...
best_configs = {}
with open('mi350_bench.csv') as csvfile:
    ...
    for row in reader:
        ...
        counters[(config_id, padding_config, counter_name)] = counter_val
    ...
    for config_id, padding_config in configs:
        if config_id not in best_configs:
            best_configs[config_id] = [["", 1e10], ["", 1e10], ["", 1e10]]
        conflicts = counters[(config_id, padding_config, "SQ_LDS_BANK_CONFLICT")]
        stalls = counters[(config_id, padding_config, "SQ_LDS_UNALIGNED_STALL")]
        total = conflicts + stalls
        if best_configs[config_id][0][1] == conflicts:
            best_configs[config_id][0][0] += [padding_config]
        if best_configs[config_id][1][1] == stalls:
            best_configs[config_id][1][0] += [padding_config]
        if best_configs[config_id][2][1] == total:
            best_configs[config_id][2][0] += [padding_config]

        if best_configs[config_id][0][1] > conflicts:
            best_configs[config_id][0] = [[padding_config], conflicts]
        if best_configs[config_id][1][1] > stalls:
            best_configs[config_id][1] = [[padding_config], stalls]
        if best_configs[config_id][2][1] > total:
            best_configs[config_id][2] = [[padding_config], total]
    for config in best_configs:
        ...
        print(config_info[config], "total cycles:", best_configs[config][2])
