...
THREADS_PER_WARP: ttgl.constexpr = triton.language.constexpr(
    triton.runtime.driver.active.get_current_target().warp_size)
...
def get_autotune_config():
    ...
    return [triton.Config(s, num_warps=nw, num_stages=1) for s in sizes for nw in [1, 2, 4]]
...
def matmul_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    A_LOAD_LAYOUT: ttgl.constexpr = ttgl.BlockedLayout(size_per_thread=[1, 1, SUB_BLOCK_SIZE_K],
                                                       threads_per_warp=[1, NUM_SUB_BLOCK_K, 1],
                                                       warps_per_cta=[ttgl.num_warps(), 1, 1], order=[2, 1, 0])
    B_LOAD_LAYOUT: ttgl.constexpr = ttgl.BlockedLayout(size_per_thread=[1, SUB_BLOCK_SIZE_K,
                                                                        1], threads_per_warp=[NUM_SUB_BLOCK_K, 1, 1],
                                                       warps_per_cta=[ttgl.num_warps(), 1, 1], order=[1, 0, 2])
    ACC_LAYOUT: ttgl.constexpr = ttgl.BlockedLayout(size_per_thread=[1, 1, 1], threads_per_warp=[NUM_SUB_BLOCK_K, 1, 1],
                                                    warps_per_cta=[1, ttgl.num_warps(), 1], order=[2, 1, 0])

    SHARED_LAYOUT: ttgl.constexpr = ttgl.SwizzledSharedLayout(1, 1, 1, order=[2, 1, 0])

    offs_innerk = ttgl.arange(0, SUB_BLOCK_SIZE_K, ttgl.SliceLayout(0, ttgl.SliceLayout(2, B_LOAD_LAYOUT)))
    ...
    b_ptrs = b_ptr + offs_outerk[:, None, None] * stride_bk * SUB_BLOCK_SIZE_K + offs_innerk[
        None, :, None] * stride_bk + offs_bn[None, None, :] * stride_bn

    MAX_K: ttgl.constexpr = 16 * 1024
    b_smem = ttgl.allocate_shared_memory(b_ptr.dtype.element_ty,
                                         [MAX_K // BLOCK_SIZE_K, NUM_SUB_BLOCK_K, SUB_BLOCK_SIZE_K, N], SHARED_LAYOUT)
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        b = b_smem.index(k).load(RHS_LAYOUT).to(ttgl.float16)
