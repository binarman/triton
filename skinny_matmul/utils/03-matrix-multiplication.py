...
@gluon.jit
def gluon_skinny_matmul_unrolled_kernel(
        # Pointers to matrices
        a_ptr, b_ptr, c_ptr,
        # Matrix dimensions
        M, N, K,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K * 2)):
        ...
        a = ttgl.load(
            a_ptrs, mask=offs_ak_sub_block[None, :, None] * SUB_BLOCK_SIZE_K + offs_ak[None, None, :]
            < K - (k) * BLOCK_SIZE_K * 2 - BLOCK_SIZE_K, other=0.0)
        b = ttgl.load(
            b_ptrs, mask=offs_bk_sub_block[:, None, None] * SUB_BLOCK_SIZE_K + offs_bk[None, :, None]
            < K - (k) * BLOCK_SIZE_K * 2 - BLOCK_SIZE_K, other=0.0)
        a = a.permute(1, 0, 2)
...
def matmul_unrolled_fma(a, b, BLOCK_SIZE_M, activation=""):
    # Check constraints.
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    assert a.is_contiguous(), "Matrix A must be contiguous"
    M, K = a.shape
    ...
    pgm = gluon_skinny_matmul_unrolled_kernel[grid](
        # matmul_kernel[grid](
        a, b, c,  #
        M, N, K,  #
        a.stride(0), a.stride(1),  #
        ...
        BLOCK_SIZE_M=BLOCK_SIZE_M,  #
        THREADS_PER_WARP=THREADS_PER_WARP)
    config_id = 3000 + BLOCK_SIZE_M
    global reported_configs
    if config_id not in reported_configs:
        print("UNROLLED FMA with BLOCK_SIZE_M: ", BLOCK_SIZE_M)
        reported_configs[config_id] = True
        for line in pgm.asm["amdgcn"].split("\n"):
            if "gpr_count" in line or "spill_count" in line:
                print(line)
