...
class _attention(torch.autograd.Function):
    ...
    def forward(cls, q, k, v, scale_float, matrix_instr_nonkdim=0):
        ...
        #print(grid)
        #print(_strides(k, "kz", "kn", "kg", "kh", "kk"))
        #print("BLOCK_N", BLOCK_N)

        pgm = _fwd_kernel_splitK[grid](
            Q=q,
            K=k,
            V=v,
        ...
        #print("reduce split", split_k, k_block_size, k_block_num)
        _splitK_reduce[grid](
            o_splitk,
            metadata,
            out,
...
def test_op_fwd(B, Mq, Mkv, Hq, Hkv, K, matrix_instr_nonkdim, dtype=torch.float16):
    ...
    # compare
    torch.testing.assert_close(ref_out, tri_out, atol=1e-3, rtol=0.01)
