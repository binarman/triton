...
def compute_scratch_buffer_shape(src_layout, dst_layout):
    src_tile = compute_rep_shape(src_layout)
    dst_tile = compute_rep_shape(dst_layout)
    return np.maximum(src_tile, dst_tile)
...
def test_convert2d(M, N, src_layout, interm_layout, dst_layout, dtype, device):
    ...
    if str(src_layout) == str(dst_layout):
        pytest.skip()
    if is_hip():
        scratch_shape = compute_scratch_buffer_shape(src_layout, dst_layout)
        lds_size = 65536
        int32_size = 4
