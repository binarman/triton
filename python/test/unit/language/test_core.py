...
def test_lds_roundtrip(device, tmp_path: pathlib.Path):
    ir = """
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx942", "ttg.threads-per-warp" = 64 : i32} {
  tt.func public @kernel(%arg0: !tt.ptr<f8E4M3FNUZ> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}, %arg1: !tt.ptr<f32> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}) attributes {noinline = false} {
    ...
    shape = (16, 32)
    src = torch.zeros(shape, dtype=torch.int8, device=device)
    for i in range(shape[0]):
        for j in range(shape[1]):
            src[i, j] = (i * shape[1] + j) % 256 - 128
    dst = torch.zeros(shape, dtype=torch.float32, device=device)
