...
int getCvtOpLDSUsage(RankedTensorType srcTy, RankedTensorType dstTy) {
  ...
  auto bytes =
      mlir::isa<triton::PointerType>(srcTy.getElementType())
...
Attribute createTmpLayout(Attribute layout, ArrayRef<unsigned> warpsPerCTA) {
  ...
  if (auto src = mlir::dyn_cast<triton::gpu::AMDMfmaEncodingAttr>(layout))
    return triton::gpu::AMDMfmaEncodingAttr::get(
        ...
        src.getCTALayout());
  if (auto src = mlir::dyn_cast<triton::gpu::AMDWmmaEncodingAttr>(layout))
    return triton::gpu::AMDWmmaEncodingAttr::get(ctx, warpsPerCTA,
                                                 src.getCTALayout());
  if (auto src = mlir::dyn_cast<triton::gpu::BlockedEncodingAttr>(layout))
  ...
  if (auto src = mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(layout)) {
    return triton::gpu::DotOperandEncodingAttr::get(
        ctx, src.getOpIdx(), createTmpLayout(src.getParent(), warpsPerCTA),
        src.getKWidth());
  }
  if (auto src = mlir::dyn_cast<triton::gpu::SliceEncodingAttr>(layout))
