...
public:
  ...
  mlir::LogicalResult
  tryAtomicRMW(mlir::Operation *op,
                  mlir::PatternRewriter &rewriter) const {

    auto stOp = dyn_cast<triton::AtomicRMWOp>(op);
    if (!stOp)
      return mlir::failure();
    Value ptr = stOp.getPtr();
    Value val = stOp.getVal();
    Value mask = stOp.getMask();
    auto ptrType = ptr.getType().dyn_cast<RankedTensorType>();
    auto valType = val.getType().dyn_cast<RankedTensorType>();
    if (!ptrType || !valType ||
    ...
    rewriter.replaceOpWithNewOp<triton::AtomicRMWOp>(
        stOp, stOp.getResult().getType(), stOp.getAtomicRmwOpAttr(), newPtr, newVal, newMask, stOp.getSemAttr());
    return mlir::success();
  }
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    auto res = tryStore(op, rewriter);
    if (res.succeeded())
      return res;
    return tryAtomicRMW(op, rewriter);
  }
};
