// RUN: (! triton-opt %s -split-input-file --tritonamdgpu-accelerate-matmul=matrix-core-version=2 --mlir-pass-pipeline-crash-reproducer=%t 2>/dev/null) | FileCheck --check-prefixes=CHECK %s
