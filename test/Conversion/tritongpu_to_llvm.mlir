...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 1 : i32} {
  ...
  tt.func @vectorized_load_f16(%a_ptr_init: tensor<256x!tt.ptr<f16>, #blocked0>, %cst : tensor<256xi1, #blocked0>, %cst_0 : tensor<256xf16, #blocked0>) {
    ...
    // GCN: llvm.insertvalue {{.*}}[3] : !llvm.struct<(f16, f16, f16, f16, f16, f16, f16, f16)>
    // GCN: llvm.insertvalue {{.*}}[4] : !llvm.struct<(f16, f16, f16, f16, f16, f16, f16, f16)>
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32} {
  ...
  tt.func @store_with_cache_attr(%a_ptr_init : tensor<256x!tt.ptr<f32>, #blocked0>, %cst : tensor<256xi1, #blocked0>, %cst_0 : tensor<256xf32, #blocked0>) {
    //      PTX: llvm.inline_asm
    // PTX-SAME: st.global.L1::evict_last.b32
    //      PTX: llvm.inline_asm
    // PTX-SAME: st.global.L1::evict_last.b32
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 2 : i32} {
  ...
  tt.func @global_load_store_no_vec(%arg0: !tt.ptr<f32> {tt.divisibility = 4 : i32}, %arg1: !tt.ptr<f32> {tt.divisibility = 4 : i32}, %arg2: !tt.ptr<f32> {tt.divisibility = 4 : i32}, %arg3: i32) {
    ...
    // GCN-NOT: llvm.inline_asm
    // GCN: llvm.extractvalue {{.*}}[0] : !llvm.struct<(f32, f32, f32, f32)>
...
#mfma0 = #triton_gpu.mfma<{mDim = 32, nDim = 32, warpsPerCTA=[1,1], isTranspose=false, CTAsPerCGA = [1, 1], CTASplitNum = [1, 1], CTAOrder = [1, 0]}>
...
#mfma = #triton_gpu.mfma<{mDim = 32, nDim = 32, warpsPerCTA = [2, 2], isTranspose=false, CTAsPerCGA = [1, 1], CTASplitNum = [1, 1], CTAOrder = [1, 0]}>
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 1 : i32} {
  // CHECK: llvm.mlir.global external @global_smem() {addr_space = 3 : i32} : !llvm.array<0 x i8>
  ...
  tt.func @convert_layout_mfma_blocked(%arg0: tensor<32x32xf32, #mfma>) {
    ...
    %0 = triton_gpu.convert_layout %arg0 : (tensor<32x32xf32, #mfma>) -> tensor<32x32xf32, #blocked0>
    tt.return
  }
}
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 1 : i32} {
  ...
  tt.func @convert_layout_mmav2_blocked(%arg0: tensor<32x16xf32, #mma>) {
    ...
    // PTX: nvvm.barrier0
    // PTX: llvm.load
    // PTX-SAME: !llvm.ptr<vector<4xf32>, 3>
...
#shared = #triton_gpu.shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [1, 0], CTAsPerCGA = [1, 1], CTASplitNum = [1, 1], CTAOrder = [1, 0]}>
#mfma = #triton_gpu.mfma<{mDim = 32, nDim = 32, warpsPerCTA = [2, 2], isTransposed=false, CTAsPerCGA = [1, 1], CTASplitNum = [1, 1], CTAOrder = [1, 0]}>
