...
def _get_a8w8_configs():
    a8w8_configs = [
        ...
        triton.Config({'BLOCK_M': 16, 'BLOCK_N': 128, 'BLOCK_K': 512, 'GROUP_SIZE_M': 1, 'matrix_instr_nonkdim': 16, 'kpack': 2}, num_stages=0, num_warps=8),
...
# for tuning config BLOCK_K=1024/512/256, BLOCK_N=128, num_warps=8, and kpack=2
# instruction size mfma16x16x32_int8
def shuffle_weight_tensor(BN, BK, N, K, b):
    mfma_k_threads = 4
    mfma_nonk_threads = 16
    mfma_size = 16
    k_width = 16 # number of elements per thread per one mfma tile
